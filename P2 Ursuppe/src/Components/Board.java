...
public class Board extends JPanel {
	...
	private void initBoard() {
		...
		this.setScoreLadder();
		this.arrayBoard=new ISquare[14][14]; //[y][x] x=width y=height //WARNIG IMPORTANT: first y then x!!!!!!
		
		this.createStringBoard();
	}
	...
	private void createStringBoard()
	{
		this.initArrayBoard();
		
		createLadderArrayBoard();
		
		this.createSoupSquareArrayBoard(4,4);
		
		this.translateArrayBoardIntoStringBoard();
		
		//System.out.println("TEST OUTPUT BOARD: \n" +  this.getBoardAsString());
	}
	
	private void createSoupSquareArrayBoard(int xStart , int yStart)
	{
		for (int x=xStart; x<this.board.length+xStart;x++)
		{
			for (int y=yStart; y<this.board.length+yStart;y++)
			{
				arrayBoard[x][y]=this.board[y-yStart][x-xStart];
			}
		}
	...
	/**
	 * reset the arrayBoard and filled in each cell with emptySquare
	 */
	private void initArrayBoard()
	{
		for (int x=0; x<arrayBoard.length;x++)
		{
			for (int y=0; y<arrayBoard.length;y++)
			{
				arrayBoard[x][y]=new EmptySquare();
			}
		}
	}


	private void createLadderArrayBoard() {
		
		int sum=0;
		
		int indexOfLadderSquare=1;

		

		for (int x=0; x<14; x++)
		{
			arrayBoard[0][x]=this.getLadderSquare(indexOfLadderSquare);
			indexOfLadderSquare++;
			sum++;
		}
		...
		for (int y=1; y<14; y++)
		{
			arrayBoard[y][13]=this.getLadderSquare(indexOfLadderSquare);
			indexOfLadderSquare++;
			sum++;
		}
		...
		for (int x=13; x>0; x--)
		{
			arrayBoard[13][x]=this.getLadderSquare(indexOfLadderSquare);
			indexOfLadderSquare++;
			sum++;
		}
		...
		for (int y=13;y>0;y--)
		{
			if (indexOfLadderSquare<this.getLadder().size())
			{
				arrayBoard[y][0]=this.getLadderSquare(indexOfLadderSquare);
				indexOfLadderSquare++;
				sum++;
			}
			else
			{
				//fill with empty
				arrayBoard[y][0]=new EmptySquare();
			}
	...
	/**
	 * this method must be called every time if the board changed!
	 * @param arrayBoard
	 */
	private void translateArrayBoardIntoStringBoard()
	{
		this.boardString="\n";
		...
		for (int x=0; x<arrayBoard.length;x++)
		{
			for (int y=0; y<arrayBoard.length;y++)
			{
				this.boardString=this.boardString+arrayBoard[x][y].getUpperStringPart();
			}
			...
			for (int y=0; y<arrayBoard.length;y++)
			{
				this.boardString=this.boardString+arrayBoard[x][y].getMiddleStringPart();
			}
			...
			for (int y=0; y<arrayBoard.length;y++)
			{
				this.boardString=this.boardString+arrayBoard[x][y].getDownerStringPart();
			}
	...
	{
		translateArrayBoardIntoStringBoard();
